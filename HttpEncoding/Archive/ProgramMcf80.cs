...
public class GetSocketPort80Mcf
{
    ...
    private static string SocketSendReceive(string server, int port)
    {
        ...
        int byteChunk = 4096; //-- 2048;
        string request = "GET / HTTP/1.1\r\nHost: " + server +
            "\r\nConnection: Close\r\n\r\n";
        Byte[] bytesSent = Encoding.ASCII.GetBytes(request);
        Byte[] bytesReceived = new Byte[byteChunk];
        Byte[] bytesRec3 = new Byte[byteChunk];
        ...
        using (Socket s = ConnectSocket(server, port))
        {
            ...
            do
            {
                ...
                //if (bytes == 0)
                //{
                //    int bytes2 = s.Receive(bytesReceived, bytesReceived.Length, SocketFlags.None);
                //    int bytes3 = s.Receive(bytesRec3, bytesRec3.Length, SocketFlags.None);
                //    //--int bytes4 = s.Receive(bytes4x, bytes4x.Length, SocketFlags.None);
                //    part2 = Encoding.ASCII.GetString(bytesRec3, 0, byteChunk);
                //}
                if (bytes < byteChunk)
                {
